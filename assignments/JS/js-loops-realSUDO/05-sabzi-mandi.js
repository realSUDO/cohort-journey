...
export function sabziMandiBill(shoppingList, priceList) {
	...
	for (let el of shoppingList) {
		let price = priceList[el.name]
		if (!Object.keys(priceList).includes(el.name) || price > 80) continue;
		price = el.qty * price
		items.push({ name: el.name, qty: el.qty, cost: price })
...
// if sabzi --> greater than 80 don't buy.. 

var lasdjfk = sabziMandiBill([{ name: "aloo", qty: 2 }, { name: "shimla mirch", qty: 1 }], { aloo: 30, tamatar: 40, shimla_mirch: 90 })
console.log(lasdjfk)
