...
export function railwayReservation(passengers, trains) {
	if (!Array.isArray(passengers) || !Array.isArray(trains) || !passengers || !trains || passengers.length === 0 || trains.length === 0)
		return [];

	let result = [];
	for (let passenger of passengers) {
		let ticket = { name: passenger.name, trainNumber: passenger.trainNumber, class: null, status: "train_not_found" };
		let trainFound = false;

		for (let rail of trains) {
			if (rail.trainNumber === passenger.trainNumber) {
				trainFound = true;
				let prefClass = passenger.preferred;
				let fallbackClass = passenger.fallback;

				if (rail.seats[prefClass] > 0) {
					ticket.class = prefClass;
					ticket.status = "confirmed";
					rail.seats[prefClass]--;
				} else if (rail.seats[fallbackClass] > 0) {
					ticket.class = fallbackClass;
					ticket.status = "confirmed";
					rail.seats[fallbackClass]--;
				} else {
					ticket.class = prefClass;
