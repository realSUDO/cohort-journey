...
export function diwaliLightsPlan(lightStrings, budget) {
	...
	for (let el of selected) {
		// calculate the cost of light 
		let newKey = "cost"
		el[newKey] = Object.keys(price).includes(el.color) ? price[el.color] * el.length : 35 * el.length 
...
let adslkf = diwaliLightsPlan([{ color: "golden", length: 5 }, { color: "white", length: 10 }, { color: "multicolor", length: 3 }], 400)
console.log()
